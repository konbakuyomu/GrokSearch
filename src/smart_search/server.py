...
try:
    ...
    from smart_search.logger import log_info
    from smart_search.config import config
    from smart_search.sources import SourcesCache, merge_sources, new_session_id, split_answer_and_sources
    from smart_search.planning import engine as planning_engine, _split_csv
except ImportError:
...
def _planning_session_error(session_id: str) -> str:
    ...
    return json.dumps({
        ...
        "expected_phase_order": [
            "intent_analysis", "complexity_assessment", "query_decomposition",
            "search_strategy", "tool_selection", "execution_order",
        ],
...
# ─── Exa tools (our unique addition) ─────────────────────────────────────────

@mcp.tool(
    name="exa_search",
    output_schema=None,
    description="""
    Performs a precise, source-first web search using the Exa neural search engine.

    Best suited for: official documentation, API references, product pages, research papers,
    company information, and any task where low-noise, high-quality results matter.
    Prefer this over web_search (Grok) when you need canonical source retrieval or direct
    page-text extraction. Prefer web_search for breaking news, X/Twitter dynamics, or
    broad multi-source live synthesis.
    ...
    search_type : str
        Search mode: "neural" (semantic, default), "keyword" (traditional), or "auto" (let Exa decide).
    include_text : bool
        If True, include full page text in each result (default: False).
    include_highlights : bool
        If True, include key highlight snippets from each result (default: False).
    start_published_date : str
        ISO date string (e.g. "2024-01-01T00:00:00.000Z") to filter results published after this date.
    include_domains : str
        Comma-separated domain whitelist (e.g. "docs.python.org,developer.mozilla.org").
    exclude_domains : str
        Comma-separated domain blacklist.
    category : str
        Filter by category: "company", "research paper", "news", "pdf", "github", "tweet", "personal site", etc.

    Returns
    -------
    str
        JSON with: ok, query, search_type, results[], total, elapsed_ms.
        Each result contains: id, title, url, publishedDate, author, score,
        and optionally text, highlights, image, favicon.
    """,
)
async def exa_search(
    ...
    if not api_key:
        return '{"ok": false, "error": "EXA_API_KEY 未配置。请在 MCP 环境变量中设置 EXA_API_KEY。"}'
...
@mcp.tool(
    ...
    output_schema=None,
    description="""
    Finds web pages similar to a given URL using Exa's neural similarity search.

    Useful for discovering related resources, alternative documentation, competing products,
    or similar content across the web.

    Parameters
    ----------
    url : str
        The reference URL to find similar pages for.
    num_results : int
    ...
    -------
    str
        JSON with: ok, url, results[], total, elapsed_ms.
    """,
)
async def exa_find_similar(url: str, num_results: int = 5, ctx: Context = None) -> str:
    api_key = config.exa_api_key
    if not api_key:
        return '{"ok": false, "error": "EXA_API_KEY 未配置。请在 MCP 环境变量中设置 EXA_API_KEY。"}'

    provider = ExaSearchProvider(config.exa_base_url, api_key, config.exa_timeout)
    return await provider.find_similar(url=url, num_results=num_results, ctx=ctx)
...
@mcp.tool(
    ...
    output_schema=None,
    description="""
    Returns current Smart Search MCP server configuration and tests API connectivity.
...
async def get_config_info() -> str:
    ...
    config_info["grok_connection_test"] = test_result

    # Exa 连通性测试
    exa_test = {"status": "未测试", "message": ""}
    ...
    if exa_key:
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                exa_time = round((time.time() - exa_start) * 1000, 2)
                if resp.status_code == 200:
                    exa_test["status"] = "✅ 连接成功"
                    exa_test["message"] = f"Exa API 可用 (HTTP 200)"
                    exa_test["response_time_ms"] = exa_time
                else:
                    exa_test["status"] = "⚠️ 连接异常"
                    exa_test["message"] = f"HTTP {resp.status_code}: {resp.text[:100]}"
                    exa_test["response_time_ms"] = exa_time
        except httpx.TimeoutException:
            exa_test["status"] = "❌ 连接超时"
            exa_test["message"] = "Exa API 请求超时"
        except Exception as e:
            exa_test["status"] = "❌ 测试失败"
            exa_test["message"] = str(e)
    else:
        exa_test["status"] = "⚠️ 未配置"
        exa_test["message"] = "EXA_API_KEY 未设置，Exa 搜索功能不可用"

    config_info["exa_connection_test"] = exa_test

    # Tavily 连通性测试
    tavily_test = {"status": "未测试", "message": ""}
    ...
    if tavily_key:
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                tavily_time = round((time.time() - tavily_start) * 1000, 2)
                if resp.status_code == 200:
                    tavily_test["status"] = "✅ 连接成功"
                    tavily_test["message"] = f"Tavily API 可用 (HTTP 200)"
                    tavily_test["response_time_ms"] = tavily_time
                else:
                    tavily_test["status"] = "⚠️ 连接异常"
                    tavily_test["message"] = f"HTTP {resp.status_code}: {resp.text[:100]}"
                    tavily_test["response_time_ms"] = tavily_time
        except httpx.TimeoutException:
            tavily_test["status"] = "❌ 连接超时"
            tavily_test["message"] = "Tavily API 请求超时"
        except Exception as e:
            tavily_test["status"] = "❌ 测试失败"
            tavily_test["message"] = str(e)
    else:
        tavily_test["status"] = "⚠️ 未配置"
        tavily_test["message"] = "TAVILY_API_KEY 未设置，Tavily 功能不可用"

    config_info["tavily_connection_test"] = tavily_test

    # Firecrawl 连通性测试（仅检查 key 配置，不发实际请求）
    firecrawl_test = {"status": "未测试", "message": ""}
    if config.firecrawl_api_key:
        firecrawl_test["status"] = "✅ 已配置"
        firecrawl_test["message"] = "FIRECRAWL_API_KEY 已设置"
    else:
        firecrawl_test["status"] = "⚠️ 未配置"
        firecrawl_test["message"] = "FIRECRAWL_API_KEY 未设置，Firecrawl 功能不可用"

    config_info["firecrawl_connection_test"] = firecrawl_test
...
@mcp.tool(
    ...
    **Key Features:**
        ...
        - **Persistent Storage:** Model preference saved to ~/.config/smart-search/config.json.
