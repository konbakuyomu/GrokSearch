...
class PlanningEngine:
    ...
    def process_phase(
        ...
        level = session.complexity_level
        if level == 1 and target in ("search_strategy", "tool_selection", "execution_order"):
            return {
                "error": f"Level 1 planning completes after query_decomposition. Phase '{target}' is not available.",
                "session_id": session.session_id,
                "complexity_level": level,
            }
        if level == 2 and target == "execution_order":
            return {
                "error": f"Level 2 planning completes after tool_selection. Phase '{target}' is not available.",
                "session_id": session.session_id,
                "complexity_level": level,
            }
        # --- End phase ordering guards ---
